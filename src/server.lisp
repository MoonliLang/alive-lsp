...
(defun get-message-handlers (state)
    (list (cons "initialize" (lambda (deps msg) (declare (ignore deps)) (alive/session/handler/init:request msg)))
          ...
          (cons "$/alive/traceFunction" (lambda (deps msg)
                                            (threads:run-in-thread deps state (or (cdr (assoc :method msg)) "traceFunction")
                                                                   (cdr (assoc :id msg))
                                                                   (lambda ()
          ...
          (cons "$/alive/untraceFunctionByName" (lambda (deps msg)
                                                    (threads:run-in-thread deps state (or (cdr (assoc :method msg)) "untraceFunction")
                                                                           (cdr (assoc :id msg))
                                                                           (lambda ()
                                                                               (alive/session/handler/traced-fns:untrace-fn-by-name deps msg)))))
          (cons "$/alive/tracePackage" (lambda (deps msg)
                                           (threads:run-in-thread deps state (or (cdr (assoc :method msg)) "tracePackage")
                                                                  (cdr (assoc :id msg))
                                                                  (lambda ()
                                                                      (alive/session/handler/traced-fns:trace-pkg deps msg)))))
          (cons "$/alive/untracePackage" (lambda (deps msg)
                                             (threads:run-in-thread deps state (or (cdr (assoc :method msg)) "untracePackage")
                                                                    (cdr (assoc :id msg))
                                                                    (lambda ()
                                                                        (alive/session/handler/traced-fns:untrace-pkg deps msg)))))
...
(defun create-deps (&key input-stream output-stream state)
    (let ((handlers (get-message-handlers state)))
        (deps:create :msg-handler (lambda (deps msg) (alive/session/message:handle deps state handlers msg))
                     ...
                     :trace-pkg (lambda (pkg-name) (alive/sys/traced-fns:trace-pkg pkg-name))
