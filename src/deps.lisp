...
(defstruct dependencies
    ...
    (eval-fn nil :type (or null (function (T (or null keyword)) *)))
    ...
    (trace-pkg nil :type (or null (function (string) boolean)))
    (untrace-pkg nil :type (or null (function (string) boolean)))
...
(declaim (ftype (function (&key (:msg-handler (function (dependencies cons) (values (or null hash-table) &optional)))
                                ...
                                (:trace-pkg (function (string) boolean))
                                (:untrace-pkg (function (string) boolean))
                                ...
                                (:eval-fn (function (stream (or null keyword)) *))
...
(defun create (&key (msg-handler (lambda (deps msg) (declare (ignore deps msg))))
                    ...
                    (trace-pkg (lambda (pkg-name) (declare (ignore pkg-name)) nil))
                    (untrace-pkg (lambda (pkg-name) (declare (ignore pkg-name)) nil))
                    ...
                    (eval-fn (lambda (s k) (declare (ignore s k))))
...
(declaim (ftype (function (dependencies T (or null keyword)) *) do-eval))
(defun do-eval (deps data lang)
    (let ((results (multiple-value-list (funcall (dependencies-eval-fn deps) data lang))))
        (finish-output)
        results))
